...
"""Return the approximate starting pixel locations when imaging an ROI.

TODO: this doesn't support other than 1024x1024 notwithstanding the crop.
"""
function calc_imaging_roi(roi_minX, roi_maxX, roi_minY, roi_maxY;
    room = "B118", W = 1024, H = 1024)
    (minX, maxX), (minY, maxY) = markpoints_magic_numbers(room)
    x = (roi_minX - minX) / (maxX - minX) * W
    y = (roi_minY - minY) / (maxY - minY) * H
    end_x = (roi_maxX - minX) / (maxX - minX) * W
    end_y = (roi_maxY - minY) / (maxY - minY) * H
    w = end_x - x
    h = end_y - y
    x, y, w, h
end

function getImagingROI(prairieViewXML, room = "B118")
    roi_xmin = parse(Float64,
        prairieViewXML[xpath"/PVScan/PVStateShard[1]/PVStateValue[@key='minVoltage']/IndexedValue[@index='XAxis']/@value"][1])
    roi_ymin = parse(Float64,
        prairieViewXML[xpath"/PVScan/PVStateShard[1]/PVStateValue[@key='minVoltage']/IndexedValue[@index='YAxis']/@value"][1])
    roi_xmax = parse(Float64,
        prairieViewXML[xpath"/PVScan/PVStateShard[1]/PVStateValue[@key='maxVoltage']/IndexedValue[@index='XAxis']/@value"][1])
    roi_ymax = parse(Float64,
        prairieViewXML[xpath"/PVScan/PVStateShard[1]/PVStateValue[@key='maxVoltage']/IndexedValue[@index='YAxis']/@value"][1])
    calc_imaging_roi(roi_xmin, roi_xmax, roi_ymin, roi_ymax,
        room = room)
end
...
default args prior to 2022-02-02: maxX=4.17170481, maxY=4.636140912,room="B115"
"""
...
function read_gpl(gpl_xml::ETree; zoom = 1.0, width = 1024, height = 1024,
    room = "B118")
...
function write_trial_order(trial_order, outname)
    trialOrderDF = DataFrame(copy(hcat(collect(1:length(trial_order)), trial_order)'), :auto)

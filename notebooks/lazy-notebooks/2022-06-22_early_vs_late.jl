...
function dorsal_ventral_hab_df(recording)
    @pun (region_masks_h5, imaging2zseries_plane, zbrain_mask_names,
        ...
        trial_order, tseriesH, tseriesW, tseriesZ) = recording
...
plot_dir = "/home/tyler/Dropbox/Science/manuscripts/2022_chrmine-fish/2021-09-09_regions"
mkpath(plot_dir)
...
# titration_plot_df = Lensman.add_timeperiod_to_df(titration_plot_df);
titration_plot_df = add_timeperiods_to_df(titration_plot_df);
catarr = CategoricalArray(Array(titration_plot_df.period) .* " " .* Array(titration_plot_df.timeperiod), ordered=true)
levels!(catarr, ["1 early", "2-6 early", "7-11 early", "12-16 early", "1 late", "2-6 late", "7-11 late", "12-16 late"])
titration_plot_df.num_cells = catarr
...
fig = Figure(resolution=(6000, 5000))
...
for (ii, major) in enumerate(sort(major_regions))
    ...
    pdf = DataFrames.combine(groupby(pdf, [:subregion, :num_cells]),
    ...
    #     mapping(:subregion, "Δf/f", color=:num_cells, dodge=:num_cells)
    p2 = Data(pdf) * visual(CrossBar) *
         mapping(:subregion, "Δf/f", "minus_sem", "plus_sem", color=:num_cells, dodge=:num_cells)
    # mapping(:region, "Δf/f", color=:num_cells, dodge=:num_cells)
    # p = p1 + p2
    p = p2
    # push!(ps, p)
...
# mapping(:region, "Δf/f", color=:num_cells, dodge=:num_cells, row="major region")
...
for n in sort(setdiff(region_names[significant5], region_names[significant1]))
    println(n)
end
println("=====Significant at p=0.01: $(sum(significant1))/$(length(pvals))=====")
for n in sort(region_names[significant1])
    println(n)
end

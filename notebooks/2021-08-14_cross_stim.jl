# basically a copy of the 2021-03-18 notebook
# small ROI shows higher wtChrmine ramping
# 1-plane 512x512 shows higher or equal wtChrmine ramping
# 10plane shows higher ramping from rsChRmine...
## analyze cross-stim from imaging laser
ENV["DISPLAY"] = "localhost:10"
using Lensman, ImageView, Glob, LibExpat, Statistics, ProgressMeter, DataFrames,
...
# matched...
gcampDir = "/data/dlab/b115/2021-03-16_h2b6s/fish1"
h33rDir = "/data/dlab/b115/2021-03-16_h33r-chrmine_h2b6s/fish4"
rsDir = "/data/dlab/b115/2021-03-16_rschrmine_h2b6s/fish3/"
wtDir = "/data/dlab/b115/2021-03-16_wt-chrmine_h2b6s/fish2/"
...
plotDir = "/home/tyler/Dropbox/Science/manuscripts/2021_chrmine-structure/cross-stim_plots"
if ~isdir(plotDir)
    mkdir(plotDir)
end
##
...
function rawToDf_f(fluor::DataFrame; before=Dict(100 => 50, 512 => 50, 5120 => 3))
    ...
    for df in groupby(fluor, [:genotype, :trial_num, :roi])
        ...
        if df.roi[1] == 100
            df = copy(df[1:50:end,:])
            kalmanFilt = resample_pad(kalmanFilt, 0.02)
            # df = copy(df[1:25:end,:])
            # kalmanFilt = resample_pad(kalmanFilt, 0.04)
        elseif df.roi[1] == 512
            df = copy(df[1:10:end,:])
            kalmanFilt = resample_pad(kalmanFilt, 0.1)
            # df = copy(df[1:5:end,:])
...
sem_plus(x) = mean(x) + sem(x)
sem_minus(x) = mean(x) - sem(x)
std_plus(x) = mean(x) + std(x)
std_minus(x) = mean(x) - std(x)
...
mean_cross_stim_df_f = combine(groupby(cross_stim_df_f[:, ["time", "df_f", "trial_num", "genotype", "roi"]],
    # ["time", "genotype", "roi"]), "df_f" => mean, "df_f" => sem_plus, "df_f" => sem_minus)
...
line = plot(df, x=:time,
    ...
    chrmine_paper,
    Geom.subplot_grid(Geom.line, Geom.ribbon,
        # Gadfly.Coord.Cartesian(ymin=-0.05, ymax=0.15)))
        ))
    # Geom.subplot_grid(Geom.line))
